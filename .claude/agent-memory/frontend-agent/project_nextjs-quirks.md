...
**CSS `var()` tokens do NOT resolve inside SVG *presentation attributes* — only in a CSS context.** `stroke="var(--color-ink)"` / `fill="var(...)"` on an SVG element silently fails to render the color; the browser parses presentation attributes as SVG typed values, not CSS.
- **How to apply:** In the hand-rolled inline-SVG charts (`components/dashboard/TrendChart.tsx`, `ResponseChart.tsx`), set token-driven colors via inline `style` instead — `style={{ stroke: 'var(--color-ink)' }}`, `style={{ fill: 'var(--color-text-muted)' }}`, `style={{ stopColor: 'var(--color-ink)' }}` on `<stop>`. `stroke`/`fill`/`stop-color` are real CSS properties there, so `var()` resolves. Numeric/opacity attrs (`strokeWidth`, `fillOpacity`, `stopOpacity`) can stay as plain attributes. This kept tsc clean.
